...
VERSION 2: Uses Water Filling Allocation (follows NSW version 2)
...
Updated for Version 2: Uses fixed dataset approach and water filling allocation (same as NSW version 2).
"""
...
# Parameters (using optimal values from Version 2 sensitivity analysis)
N_trust = 10
N_mal = 4
T = 10
c_mal = 5.0  # More realistic malicious distortion
n_trials = 20  # Number of trials to average over
# IMPORTANT: This value should match the optimal xi0 from Version 2 sensitivity analysis
# The optimal xi0 should be from the NON-CRASHED region (Option 2 recommended)
# Updated value: 0.27 (from Version 2 sensitivity analysis output - Option 2 optimal point)
xi0_optimal = 0.27  # Optimal xi0 from Version 2 sensitivity analysis (Option 2 - vs Clean, non-crashed region)
gamma = 0.1
alpha_pairwise = 5.0
...
def online_set_aside_with_trust_trajectory(v_true, reports_func_for_malicious, detect_fn, 
    ...
    for t in range(T):
        ...
        # 4) Greedy allocation using water filling (VERSION 2 – follows NSW version 2)
        # Water filling: allocate to equalize marginal utilities (reported_value / predicted_utility)
        ...
        # Compute marginal utilities for water filling (same as NSW version 2)
        ...
        # Water filling allocation: allocate proportionally to marginal utilities (follows NSW v2)
        y[:, t] = 0.0
        if n_eligible > 0:
            eligible_indices = np.where(eligible)[0]
            eligible_marginal = marginal_util[eligible_indices]
            
            total_marginal = eligible_marginal.sum()
            if total_marginal > 0:
                for idx in eligible_indices:
                    y[idx, t] = 0.5 * (marginal_util[idx] / total_marginal)
            else:
...
# ----------------------------------------------------------------------
print("=" * 70)
print("UTILITY TRAJECTORIES VISUALIZATION - VERSION 2 (Proportional Allocation)")
print("=" * 70)
print("Using optimal xi0 = {:.3f} (from Version 2 sensitivity analysis - Option 2 optimal point, non-crashed region)".format(xi0_optimal))
print("Generating {} fixed datasets (one per trial)...".format(n_trials))

# Step 1: Generate all datasets upfront with fixed seeds (consistent with Version 2)
# This ensures reproducibility and fair comparison
fixed_datasets = []
for trial in range(n_trials):
    np.random.seed(42 + trial)  # Fixed seed per trial (same as Version 2)
    N_all = N_trust + N_mal
...
for trial in range(n_trials):
    ...
    try:
        # Use fixed dataset for this trial (consistent with Version 2 methodology)
        N_all = N_trust + N_mal
...
ax.set_title('Utility Trajectories Over Time (Version 2 – Water Filling)\n'
...
plt.tight_layout()
plt.savefig('utility_trajectories_over_time_version2.png', dpi=150, bbox_inches='tight')
print("\n" + "=" * 70)
print("VISUALIZATION COMPLETE")
print("=" * 70)
print("Plot saved as 'utility_trajectories_over_time_version2.png'")
print()
