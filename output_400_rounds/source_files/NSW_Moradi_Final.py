...
[R1] Bar plots converted to line plots for all sweep/comparison figures.
     ...
     appropriate (e.g. the three-way detector F1 grouped bar).
...
class ExperimentConfig:
    ...
    n_malicious:    int   = 4
    n_rounds:       int   = 200
    n_trials:       int   = 25      # [R6] raised for deeper analysis
...
def sparsest_subgraph_detection(W: np.ndarray,
    ...
    """
    PART B — Approximate sparsest-subgraph detector (Moradi 2015, Sec. 3.2).

    Finds the subset S* ⊆ V minimising internal edge density ρ(S) = sum_W / |S|.
    Malicious agents have low trust from all observers → they form the sparsest
    subgraph. This detector does NOT assume a two-cluster partition, unlike
    spectral clustering.

    Algorithm (Bahmani et al. 2012):
      1. S ← all nodes
      2. ρ(S) ← sum of edge weights in S / |S|
      3. θ ← (2+2ε)·ρ(S)
      4. Remove top ε/(1+ε) fraction by weighted degree
      5. Track sparsest S seen; repeat until S is empty
      6. Trim final set to n_malicious by ascending degree (sparsest first)
    """
    N      = n_agents
    ...
    while len(S) >= 2:
        ...
        rho_S = W_sub.sum() / len(S)

        if rho_S < best_rho and len(S) >= max(2, n_malicious):
            best_rho = rho_S
            best_S   = list(S)

        wd         = W_sub.sum(axis=1)
        theta      = (2.0 + 2.0 * epsilon) * rho_S
        candidates = [S[k] for k, d in enumerate(wd) if d >= theta]
        if not candidates:
            candidates = [S[int(np.argmax(wd))]]

        n_remove  = max(1, int(epsilon / (1.0 + epsilon) * len(candidates)))
        cand_wd   = {node: W[node, :][np.array(S)].sum() for node in candidates}
        to_remove = set(sorted(candidates, key=lambda n: cand_wd[n],
                               reverse=True)[:n_remove])
        S = [node for node in S if node not in to_remove]

    if len(best_S) > n_malicious:
        sub_idx = np.array(best_S)
        sub_wd  = W[np.ix_(sub_idx, sub_idx)].sum(axis=1)
        best_S  = [best_S[k] for k in np.argsort(sub_wd)[:n_malicious]]
...
def spectral_detection(beta_matrix: np.ndarray,
    ...
    Limitation: assumes exactly two clusters. Fails when attack patterns
    blur the graph boundary (TRUST_MIMICRY, REPUTATION_POISON Phase 2).
    """
    ...
    c1 = int(labels.sum())
    c0 = n_agents - c1
    if abs(c1 - n_malicious) > abs(c0 - n_malicious):
        labels = 1 - labels
    return labels
...
def trusted_online_allocation(
    ...
    identified by Algorithm 1 (PART B) each round.
...
class NSWTrustworthySim:
    ...
    Integrates all three parts each round:
      1. PART B detects malicious agents via Algorithm 1 → trusted_ids
      2. PART A allocates resources to trusted_ids via water-filling
      3. PART C updates utilities, computes NSW, records statistics
    ...
    def step(self) -> Dict:
        ...
        # (A) Pre-round detection — used by adaptive attackers to calibrate
...
def convergence_round(history: List[Dict], threshold: float = 0.95) -> int:
    """First round t where detection_rate >= threshold. Returns T if never reached."""
    for row in history:
...
def plot_main_results_all_types(
    ...
    metrics = [
        ('detection_rate', 'Detection Rate',           'Detection Rate (Algorithm 1)'),
...
def plot_attack_summary_lines(
    ...
    metrics = [
        ...
        ('avg_alg1_f1', None,      'Avg Algorithm 1 F1',          '#E53935'),
    ]
...
# ----------------------------------------------------------------------------
# Plot 9: Three-way detector benchmark [R6]
# ----------------------------------------------------------------------------
...
def plot_three_way_detector(
    ...
    """
    [R6] Three-way F1 comparison: Algorithm 1 vs Spectral vs Sparsest-Subgraph
    for the 4 focused attack types. Grouped bar chart with per-method colour coding.
    """
    types    = list(results.keys())
    labels   = [ATTACK_LABELS[at] for at in types]
    ...
    ax.bar(x - w, alg1_v,   width=w, color='#1E88E5', alpha=0.85,
           label='Algorithm 1 (PART B)', edgecolor='white')
    ax.bar(x,     spec_v,   width=w, color='#FB8C00', alpha=0.85,
           label='Spectral Clustering', edgecolor='white')
    ax.bar(x + w, sparse_v, width=w, color='#43A047', alpha=0.85,
           label='Sparsest Subgraph (Moradi)', edgecolor='white')
    ...
    ax.set_ylim(0, 1.20)
    ax.set_title(
        'Three-Way Detection Benchmark: Algorithm 1 vs Spectral vs Sparsest-Subgraph\n'
...
#          whether the Moradi graph correctly identifies low-trust agents
#          before Algorithm 1 formally flags them.
# ----------------------------------------------------------------------------
...
def plot_utility_with_moradi(
    ...
    Bottom panel — Moradi blended W_ij score for each malicious agent
      ...
      agent means the Moradi graph is beginning to isolate it before
      Algorithm 1 crosses the formal threshold ξ_t.

    Vertical dashed line marks the first round where Algorithm 1
    majority-vote flags any malicious agent (detection onset).
    """
    hist   = trial['history']
    sim    = trial['sim']
    T      = len(hist)
    ...
    # Detection onset: first round any malicious agent is flagged
    detection_onset = T  # default: never
    for row in hist:
        if len(row['detected'] & set(range(N_l, N))) > 0:
            detection_onset = row['t']
            break
    ...
    if detection_onset < T:
        ax.axvline(detection_onset, color='black', ls=':', lw=1.5,
                   alpha=0.7, label=f'Detection onset (t={detection_onset})')
    ...
    ax2.axhline(0.5, color='gray', ls='--', lw=1.2, alpha=0.7,
                label='Neutral W = 0.5')
    if detection_onset < T:
        ax2.axvline(detection_onset, color='black', ls=':', lw=1.5,
                    alpha=0.7, label=f'Detection onset (t={detection_onset})')
    ...
    ax2.set_title(
        ...
        '(falling = graph isolating attacker before formal detection)',
        fontsize=10, fontweight='bold')
...
def main():
    ...
    print("\n[Phase 1] Running all four focused attack types (25 trials each) ...")
    results = compare_focused_types(base_cfg)

    # ── Plots 1–5: main diagnostics ─────────────────────────────────────────
    print("\n[Plots 1-5] Main diagnostic plots ...")
    plot_main_results_all_types(results, base_cfg)

    # Single-attack utility trajectory and heatmap (Byzantine as illustrative)
    byz_trials = results[AttackType.BYZANTINE]['trials']
    plot_utility_trajectories(byz_trials[0], replace(base_cfg, attack_type=AttackType.BYZANTINE))
    plot_allocation_heatmap(byz_trials[0],   replace(base_cfg, attack_type=AttackType.BYZANTINE))
    plot_misclassification_all_types(results, base_cfg)
    plot_attack_summary_lines(results, base_cfg)

    # ── Plot 6: β trajectory analysis ──────────────────────────────────────
    print("\n[Plot 6] β accumulation and detection threshold ...")
    plot_beta_trajectories(results, base_cfg)

    # ── Plot 7: NSW ratio sensitivity ──────────────────────────────────────
    print("\n[Plot 7] NSW ratio sensitivity to adversary count ...")
    plot_nsw_ratio_sensitivity(base_cfg)

    # ── Plot 8: Alpha sweep ─────────────────────────────────────────────────
    print("\n[Plot 8] Alpha sweep ...")
    plot_alpha_sweep(base_cfg)

    # ── Plot 9: Three-way detector benchmark ───────────────────────────────
    print("\n[Plot 9] Three-way detector benchmark ...")
    plot_three_way_detector(results)

    # ── Plot 10: Sigma sweep ────────────────────────────────────────────────
    print("\n[Plot 10] Sigma sweep ...")
    plot_sigma_sweep(base_cfg)

    # ── Plot 11: Compound attack breakdown [R4] ─────────────────────────────
    print("\n[Plot 11] Compound attack sub-type breakdown ...")
    plot_compound_breakdown(base_cfg)

    # ── Plot 12: Reputation Poison phase analysis [R3] ──────────────────────
    print("\n[Plot 12] Reputation Poison phase analysis ...")
    plot_reputation_poison_phases(base_cfg)
    ...
    print("\n[Plot 14] Attack rate analysis ...")
    plot_attack_rates(results, base_cfg)

    # ── Plot 15: Prediction scenario stress test ─────────────────────────────
    print("\n[Plot 15] Prediction scenario comparison ...")
    plot_prediction_scenario_comparison(base_cfg)
    ...
    outputs = [
        ...
        "plot9_three_way_detector.png  — Alg1 vs Spectral vs Sparsest F1",
