#!/usr/bin/env python3
import os, sys, pickle, json, csv, shutil, time
...
def main():
    ...
    for fname in ['NSW_Moradi_Final.py','approach_common.py','approach_adaptive_alpha.py','approach_pace_trusted.py','approach_generalized_mean.py','approach_sample_resolving.py','approach_expert_advice.py','approach_robust_aggregation.py','run_all_approaches_400.py','run_one_pair_400.py','collect_plot_400.py']:
        src=ROOT/fname
        if src.exists(): shutil.copy2(src,out_root/'source_files'/fname)
    all_agg=[]; all_trials=[]; errors=[]
    cfg0=r.make_config(n_rounds,n_trials)
    for approach in r.APPROACHES:
        app_dir=out_root/approach; app_dir.mkdir(exist_ok=True)
        results={at.value: [] for at in r.ATTACKS}; agg_rows=[]
        for at in r.ATTACKS:
            p=pair_dir/f'{approach}__{at.value}.pkl'
            if not p.exists():
                errors.append({'approach':approach,'attack_type':at.value,'error':'missing pair result'})
                continue
            ...
            agg_rows.append(agg); all_agg.append(agg.copy())
            for tr in rows: all_trials.append(tr['summary'])
        ...
        trial_rows=[s for s in all_trials if s['approach']==approach]
        if trial_rows: write_csv(app_dir/f'{approach}_trial_level.csv', trial_rows, list(trial_rows[0].keys()))
        try:
        ...
        print(f'PLOTTED {approach} png={len(list(app_dir.glob("*.png")))}')
    agg_fields=['approach','attack_type','n_rounds','n_trials_completed','avg_nsw','std_nsw','avg_ratio','std_ratio','avg_dr','std_dr','avg_fpr','avg_min_util','avg_fairness_gap','avg_conv','avg_alg1_f1','avg_spec_f1','avg_sparse_f1','avg_offline_nsw']
    ...
    if all_trials: write_csv(out_root/f'all_approaches_{n_rounds}rounds_trial_level.csv',all_trials,list(all_trials[0].keys()))
    # Comparison with previous 200-round summary.
    comparison=[]; previous_path=ROOT/'all_approaches_summary.csv'
    if previous_path.exists():
        ...
        for row in all_agg:
            old=prev.get((row['approach'],row['attack_type']), float('nan'))
            new=row['avg_ratio']
    ...
    try:
        ...
        for row in all_agg:
            mat[approaches.index(row['approach']), attacks.index(row['attack_type'])]=row['avg_ratio']
        ...
        for i in range(mat.shape[0]):
            for j in range(mat.shape[1]): ax.text(j,i,f'{mat[i,j]:.3f}',ha='center',va='center',fontsize=9,color='white' if mat[i,j] < np.nanmean(mat) else 'black')
