...
# ============================================================================
# MAIN: run selected Ṽ scenarios (n_trials each), report CR table
# ============================================================================

def _parse_cli_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the NSW-with-predictions reference simulation locally. "
            "Use --quick for a short local check."
        )
    )
    parser.add_argument("--agents", type=int, default=None, help="Number of agents.")
    parser.add_argument("--rounds", type=int, default=None, help="Number of periods.")
    parser.add_argument("--trials", type=int, default=None, help="Trials per scenario.")
    parser.add_argument(
        "--scenarios",
        default=None,
        help="Comma-separated prediction scenarios (default: all).",
    )
    parser.add_argument(
        "--quick",
        action="store_true",
        help=(
            "Run a small check: 5 agents, 8 rounds, 1 trial, and only ideal "
            "plus mild_noise unless explicitly overridden."
        ),
    )
    args = parser.parse_args(argv)
    args.agents = args.agents if args.agents is not None else (5 if args.quick else N)
    args.rounds = args.rounds if args.rounds is not None else (8 if args.quick else T)
    args.trials = args.trials if args.trials is not None else (1 if args.quick else n_trials)
    default_scenarios = ("ideal", "mild_noise") if args.quick else PREDICTION_SCENARIOS
    args.scenarios = (
        tuple(item.strip() for item in args.scenarios.split(",") if item.strip())
        if args.scenarios is not None
        else default_scenarios
    )
    if args.agents <= 0 or args.rounds <= 0 or args.trials <= 0:
        parser.error("--agents, --rounds, and --trials must be greater than zero")
    invalid = [item for item in args.scenarios if item not in PREDICTION_SCENARIOS]
    if invalid:
        parser.error(
            "unknown scenario(s): "
            + ", ".join(invalid)
            + "; choose from "
            + ", ".join(PREDICTION_SCENARIOS)
        )
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    print("=" * 72)
    print(
        "N = {}, T = {}, n_trials = {} per scenario".format(
            args.agents, args.rounds, args.trials
        )
    )
    print("Ṽ scenarios: " + " | ".join(args.scenarios))
    print()
    ...
    for scenario in args.scenarios:
        ...
        for trial in range(args.trials):
            r = run_experiment_once(
                args.agents,
                args.rounds,
                v_true_precomputed=None,
                trial=trial,
                scenario=scenario,
            )
            results.append(r)
    ...
    print(
        "Results (mean over {} trials per scenario); CR = Offline / Online (≥ 1)".format(
            args.trials
        )
    )
    print("-" * 72)
